  ...
  return (
    <footer className="border-t border-white/10 bg-[#050505] relative overflow-hidden py-16 px-4 sm:px-8 md:px-12 text-left">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="mb-14 p-6 sm:p-8 bg-white/5 border border-white/10 rounded-[32px] backdrop-blur-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
            {GYM_OWNERS.map((owner) => (
              <div
                key={owner.id}
                className="bg-white/5 border border-white/10 rounded-2xl p-4 sm:p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              >
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-bold text-white uppercase font-display">{owner.name}</span>
                    <span className="text-[9px] bg-[#F27D26]/20 text-[#F27D26] px-2 py-0.5 rounded-full font-bold uppercase">
                      {owner.role}
                    </span>
                  </div>
                  <p className="text-[11px] text-white/70 mt-1 font-mono">{owner.designation}</p>
                  <div className="flex items-center gap-2 mt-2 text-xs">
                    <Phone className="w-3.5 h-3.5 text-emerald-400" />
                    <a href={`tel:${owner.phoneRaw}`} className="text-white hover:text-[#F27D26] font-mono font-bold">
                      {owner.phone}
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 w-full sm:w-auto">
                  <a
                    ...
                    className="flex-1 sm:flex-none bg-gradient-to-r from-[#833ab4]/30 via-[#fd1d1d]/30 to-[#fcb045]/30 hover:from-[#833ab4] hover:via-[#fd1d1d] hover:to-[#fcb045] border border-white/15 px-3.5 py-2 rounded-full text-[11px] font-bold text-white flex items-center justify-center gap-1.5 transition-all"
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-white/5 hover:bg-emerald-600 border border-white/10 p-2 rounded-full text-white transition-all"
                    title={`WhatsApp ${owner.name}`}
                  >
                    <MessageCircle className="w-4 h-4 text-emerald-400 hover:text-white" />
                  </a>
                </div>
              </div>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-14">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-[#F27D26]/50 shadow-[0_0_20px_rgba(242,125,38,0.3)] flex-shrink-0 bg-black">
                <img
              ...
              <div>
                <span className="text-base font-black tracking-[0.15em] uppercase text-white font-display block">
                  Old Skoool Gym
                </span>
                <span className="text-[10px] uppercase tracking-widest text-[#F27D26] font-mono">
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-xs text-white/70">
              <li><a href="#trainers" className="hover:text-white transition-colors">Founders & Head Trainers</a></li>
